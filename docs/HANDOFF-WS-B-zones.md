...
- 路网底座：`HANDOFF-WS-B2.md`（QC PASS，本波不重下 B2�?- 本交付：区面贴路 v2 验收 + **05.2 情景/CI/典型路段**静�?JSON
...
见构建日�?/ `zones_shanghai.json.by_geometry_method`。目标：**road_convex_hull 占绝大多�?*�?
抽检说明（无截图文件时人工）�?
1. `npm run serve` �?开区面�? 
2. 陆家�?/ 徐家�?/ 虹桥：边界应有折线感，非正椭圆气�? 
3. 对照路网层：凸包贴主干走�? 

## 05.2 静态规则（�?D 着�?/ E 等时�?
| 文件 | 用�?|
|------|------|
| `data/static/time_scenario.json` | 6 �?time_scenario（平�?`wd_day_offpeak`，无 `wd_noon`�?|
| `data/static/scenario_ci.json` | `city_CI` + `weather_f` + `lookup_city_ci` + v0 等级 |
| `data/static/ci_series_24h.json` | 工作�?周末 24h CI（夜低·早峰·午平台·晚峰最高）+ hour→scenario |
| `data/static/typical_road_anchors.json` | 早高峰典型路段均速锚�?+ name 匹配策略 |
| `data/static/congestion_coeff.json` | 业务 difficulty�?*6×天气**�?CI 同源（v0.2�?|

全部 copy �?`public/data/*`（`copy-public-data.js`）�?
### city_CI 初值（clear�?
...
匹配：`name` 子串白名单，�?`typical_road_anchors.json`�?
## 水系
...
�?UI 默认。旧 1km `sh:{cell_m}:{row}:{col}` 仅计算兼容�?
## 如何构建

```bash
npm run build:zones
npm run copy:public-data
# 静�?JSON 无需编译，改文件�?copy 即可
```

## 下游

- **D**：两卡读 `time_scenario` + `weather`；路色用 `scenario_ci`×锚点；趋势用 `ci_series_24h`；区面用 geojson  
- **E**：等时权 `length/v`，v 来自同一 CI �? 
- **C**：difficulty 优先 `congestion_coeff` 六档�? 
